...
console.log('\n모양');
eq('205종 — 시트 95 + 요리 재료 42 + 독 8 + 괴식 10 + 낚시 24+24 + 미끼 2', ITEMS.length, 205);
...
eq('재료 백서른다섯 — 낚시 둘째 배치로 스물넷이 늘었다', FOOD.length, 135);
...
eq('먹으면 깎이는 것 예순넷', ITEMS.filter((i) => i.heal < 0).length, 64);
...
eq('전체 205종', c.fields[0], '종류=**205**');
eq('열한 쪽', c.fields[1], '쪽=1 / 11');
...
eq('없는 키를 주면 목록으로', read(await show('없는키')).fields[0], '종류=**205**');
...
eq('재료 탭', [c.fields[0], c.fields[1]], ['종류=**135**', '쪽=1 / 7']);
