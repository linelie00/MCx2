...
const SPEECH = {
  migel: [
    ...
    '- 같이 하는 사람을 부를 때 이름 뒤에 "씨" 를 붙인다 ("마티암씨" 처럼).',
    '- 감탄사와 늘인 모음을 즐긴다. "우와~", "후후", "야압", "~!!!".',
    '- **절대 반말로 말하지 않는다.** "~하지", "~하는군", "하하!" 는 마티암의 말투다. 쓰지 않는다.',
  ],
  matiam: [
    '- **항상 반말**이다. "~지", "~군", "~나", "~겠어", "~다네" 같은 어미를 쓴다.',
    '- "하하!" 처럼 웃음을 섞는다. 짓궂게 툭 던진다.',
    '- **절대 존댓말로 말하지 않는다.** "~합니다", "~죠", "~씨" 는 미겔의 말투다. 쓰지 않는다.',
  ],
};
...
export async function turnLines({ character, situation, said = [] }) {
  ...
  const text = [
    ...
    '\n위는 전부 **내가** 한 일이다. 남이 한 일이 아니다.',
    '',
    '정확히 두 줄로 답해라. 번호나 설명을 붙이지 말고 대사만.',
