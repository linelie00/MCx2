...
- **홀덤 토너먼트**(`/홀덤 토너먼트`) — **한 명이 남을 때까지.** 넷부터(모브 자리 포함) 시작하고 블라인드가
  ...
  오른다(끝판이 운 싸움이 되지 않게). 사람이 [다음 핸드] 를 안 눌러도 저절로 넘어간다. 우승자가 상금(사람·미겔·마티암이
  들고 앉은 것의 합)을 다 가져가고, **MT 는 1위 둘 · 2위 하나**. `모브:` 로 에너미를 앉힐 수 있다 —
  모브가 우승하면 상금은 에너미 몫이고, 모브를 이겨도 골드가 새로 생기지는 않는다.
