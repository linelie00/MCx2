...
const GAMES = {
  ...
  holdem: {
    doing: '카지노 "bard" 에서 텍사스 홀덤을 하는 중이다.',
    role: {
      // 홀덤에는 딜러석이 없다. 카드는 봇이 돌리고 모두가 손님이다.
      player: {
        migel: [
          '손님 자리에 앉아 카드를 받는다. bard 는 내 가게라 판을 여는 인사도 내가 한다.',
          '카드를 돌리는 쪽이 아니라 받는 쪽이라 신이 나 있다.',
        ],
        matiam: [
          '손님 자리에 앉아 카드를 받는다.',
        ],
    ...
    rules: [
      '- **Fold, Check, Call, Raise, All-in 은 영어 그대로** 쓴다. "폴드" "콜" 처럼 적지 않는다.',
      '  손 이름(플러시·풀하우스·투페어…)은 한글 그대로 쓴다.',
      '- 카드는 A, 2~10, J, Q, K 로 부른다. 무늬는 말해도 되고 안 해도 된다.',
      // 이게 이 게임의 핵심 규칙이다. 판이 전부 공개인 블랙잭과 결정적으로 다르다.
      '- **남의 두 장은 아무도 모른다.** 상대가 무엇을 들고 있는지 아는 것처럼 말하지 마라.',
      '  짐작하고 떠보는 것은 좋다 — "뭔가 잡으셨군요?" 는 되고, "에이스를 들고 계시죠" 는 안 된다.',
      '- **내 카드도 밝히지 않는다.** 무엇을 들고 있는지 말하지도, 암시하지도,',
      '  손에 쥔 것을 묘사하지도 않는다("쥔 10의 쌍을 두드리며" 같은 것도 안 된다).',
      '  셀 때 신난 티, 나쁠 때 시무룩한 티만 낸다. 카드를 깐 뒤라면 말해도 된다.',
      '- 선택지를 나열하며 묻지 않는다. 무엇을 고를 수 있는지는 화면에 이미 나와 있다.',
    ],
  },
};
