...
console.log('\n모양');
eq('251종 — 시트 95 + 요리 재료 42+22 + 독 8 + 괴식 10 + 낚시 24+24 + 미끼 2 + 농장 작물 19 + 개간 1 + 거름 3 + 귀마개 1', SHEET.length, 251);
...
eq('재료 백일흔여섯 — 장보기 둘째 배치로 스물둘, 농장 작물로 열아홉이 늘었다', FOOD.length, 176);
eq('잡화는 예순아홉 — 낚시 잡동사니 넷 · 개간 옛 동전 하나 · 거름 셋(황금 비료 포함) · 귀마개', ITEMS.filter((i) => i.kind === '잡화').length, 69);
...
eq('먹으면 깎이는 것 일흔일곱 — 농장 고추·토란·강낭콩·용의 고추, 옛 동전, 거름 셋, 귀마개', SHEET.filter((i) => i.heal < 0).length, 77);
...
eq('없는 키를 주면 목록으로', read(await show('없는키')).fields[0], '종류=**257**');
...
eq('재료 탭', [c.fields[0], c.fields[1]], ['종류=**182**', '쪽=1 / 10']);
...
// 원석 여섯 + koreanMelon(참외 — k·ore·an)
eq('키로도', (await complete('ore')).length, 7);
