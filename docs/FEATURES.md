...
| `/홀덤 던전` | 누구나 | `POST /deltas` (핸드마다 체력, 이기면 아이템) |
...
- **홀덤 토너먼트**(`/홀덤 토너먼트`) — **한 명이 남을 때까지.** 셋부터 시작하고 블라인드가
  6핸드마다 오른다. 사람이 [다음 핸드] 를 안 눌러도 저절로 넘어간다. 우승자가 판돈을 다
  가져가고 **MT 한 개**. 모브는 못 앉힌다(지갑이 없어 골드가 생기거나 사라진다).
- **던전**(`/홀덤 던전`) — 엘리트 에너미와 **단둘이, 체력을 걸고.** 스택이 곧 체력이고
  블라인드는 1/2 다. 한쪽이 0 이 되면 끝, 0 이 된 쪽은 쓰러진다. 핸드 사이에 도망칠 수
  있고, 미겔·마티암을 불러 **자기 체력으로** 대신 싸우게 할 수 있다. 이기면 잡화 2~5개.
  상대는 **일반 에너미 40종**(체력 30~60)이 기본이고 **다섯에 한 번 엘리트**(60~120)가 나온다.
