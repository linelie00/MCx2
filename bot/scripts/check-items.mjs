...
console.log('\n모양');
eq('249종 — 시트 95 + 요리 재료 42+22 + 독 8 + 괴식 10 + 낚시 24+24 + 미끼 2 + 농장 작물 18 + 개간 1 + 거름 2 + 귀마개 1', SHEET.length, 249);
...
eq('재료 백일흔다섯 — 장보기 둘째 배치로 스물둘, 농장 작물로 열여덟이 늘었다', FOOD.length, 175);
...
eq('먹으면 깎이는 것 일흔여섯 — 농장 고추·토란·강낭콩·용의 고추, 옛 동전, 거름 둘, 귀마개', SHEET.filter((i) => i.heal < 0).length, 76);
...
eq('없는 키를 주면 목록으로', read(await show('없는키')).fields[0], '종류=**255**');
...
eq('재료 탭', [c.fields[0], c.fields[1]], ['종류=**181**', '쪽=1 / 10']);
