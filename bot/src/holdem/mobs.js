 ...
 *
 * **지금은 0 이다.** "엘리트를 더 어렵게" 해서 0.09 를 걷었다 — 표의 성향 그대로 둔다.
 * 승률을 가르는 것은 체력이라(아래 hpOf) 난이도는 거기서 올렸고, 이 값은 표의 단단함을
 * 되돌리는 몫이다. 값은 남겨 둔다 — 도감의 성향이 이 값을 더해 읽는다(bestiary.js).
 */
export const ELITE_LOOSE_BONUS = 0;

/**
 * 그 에너미의 체력. 엘리트는 두껍다. 자주 마주친 놈일수록 조금 더 두껍다.
 *
 * 엘리트는 **100~160**(평균 134) — 이쪽의 최대 체력보다 두껍다. 60~120(평균 80)이었을 때
 * 승률이 61% 였고, 지금은 50% 쯤이다(판 500번, 적과 같은 엔진으로 두는 가상의 사람,
 * 블라인드 2/4 + 6핸드마다, 적의 체력 상한 있음). 판 절반이 끝나는 핸드는 10 → 15.
 */
