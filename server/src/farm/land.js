 ...
 *
 * `bot/scripts/simulate-farm.mjs` 로 맞췄다(60일 × 10판, 중앙값). 2a 에서 초안(…, 3000)의 0.35배로
 * 잡았다가, 4a 에서 날씨·계절(비·흐림·서리·폭염 · 제철이 아니면 ×0.7)이 속도를 늦춰 Lv5 부터 조금
 * 낮췄다(…, 1050 → …, 820). 이웃+먹으며 Lv10 49일 · 먹으며 55일 — 목표 6~8주.
 * **낮추기만 한다** — 올리면 운영 농장의 레벨이 내려간다. 낮춰서 레벨이 오른 농장의 밭은 읽을 때 연다
 * (`rules.upgrade`).
 * Lv2 는 나흘, Lv3 는 엿새 — 초반에 금방 밭이 늘어나는 맛이 있어야 한다.
 */
const LEVEL_XP = [0, 20, 50, 110, 170, 240, 340, 470, 630, 820];
