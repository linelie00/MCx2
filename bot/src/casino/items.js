...
export const CATS = [
  ...
  { key: 'veg', label: '채소', icon: '🥕' },
  // 과일은 **채소 바로 아래**. 한 칸에 같이 두었더니 스물셋이 되어 셀렉트 한도(25)에 닿았고,
  // 사과를 찾으려면 양파와 가지 사이를 뒤져야 했다.
  { key: 'fruit', label: '과일', icon: '🍎' },
...
export const ITEMS = [
  ...
  { key: 'softFig',         name: '무른 무화과',              kind: '재료', cat: 'fruit',  price:    1, sell: true,  heal: 2,          desc: '땅에 떨어진 지 오래되었는지 과육이 흐물거리고 단향이 진동한다. 아직 먹을 수 있다.' },
  ...
  { key: 'redApple',        name: '새빨간 사과',              kind: '재료', cat: 'fruit',  price:    5, sell: true,  heal: 10,         desc: '마녀의 집에서 주운, 새빨간 사과. ...진짜 사과일까?' },
  ...
  // ---- 낚시 둘째 배치. 기괴한 것 다섯 · 마법 붙은 것 일곱 · 이야기 넷 · 바닷것 여덟.
  { key: 'hollowEel',       name: '통장어',                  kind: '재료', cat: 'meat',  price:    3, sell: true,  heal:    5, monster: true, desc: '껍질만 남았다. 안을 들여다보면 반대쪽이 보인다.' },
  { key: 'eyeClump',        name: '눈알뭉치',                kind: '재료', cat: 'meat',  price:    6, sell: true,  heal:  -10, monster: true, desc: '작은 눈알 수십 개가 뭉쳐 한 마리처럼 헤엄친다. 전부 이쪽을 본다.' },
  { key: 'flipSide',        name: '플립사이드',              kind: '재료', cat: 'meat',  price:    4, sell: true,  heal:   -5, monster: true, desc: '배가 하늘을 본다. 죽은 게 아니라 원래 그렇다.' },
  { key: 'grinFlat',        name: '그린플랫',                kind: '재료', cat: 'meat',  price:   12, sell: true,  heal:  -15, monster: true, desc: '눈이 한쪽으로 몰린 자리에 입이 있다. 계속 웃는다.' },
  { key: 'nailCarp',        name: '손톱잉어',                kind: '재료', cat: 'meat',  price:   16, sell: true,  heal:  -20, monster: true, desc: '비늘이 전부 사람 손톱 같다. 세어 보지 않는 편이 좋다.' },
  { key: 'manaLoach',       name: '마꾸라지',                kind: '재료', cat: 'meat',  price:    8, sell: true,  heal:   10, desc: '근처에서 마법을 쓰면 몰려든다. 맛은 없다.' },
  { key: 'frostSmelt',      name: '빙빙어',                  kind: '재료', cat: 'meat',  price:   14, sell: true,  heal:   -5, desc: '건져 올리면 양동이가 먼저 언다.' },
  { key: 'emberFish',       name: '불어',                    kind: '재료', cat: 'meat',  price:   18, sell: true,  heal:  -20, poison: 1, desc: '비늘 사이에 불씨가 산다. 굽지 않아도 이미 뜨겁다.' },
  { key: 'echoCrayfish',    name: '메아리가재',              kind: '재료', cat: 'meat',  price:   15, sell: true,  heal:    5, desc: '집게를 부딪치면 어제 들은 소리가 난다.' },
  { key: 'ghostHerring',    name: '유령 청어',               kind: '재료', cat: 'meat',  price:   24, sell: true,  heal:    0, desc: '무게는 있는데 그림자가 없다.' },
  { key: 'forgetJelly',     name: '물망초 해파리',           kind: '재료', cat: 'meat',  price:   26, sell: true,  heal:  -10, desc: '만지면 방금 무엇을 하려 했는지 잊는다. 낚은 이유도.' },
  { key: 'shadowFin',       name: '섀도우핀',                kind: '재료', cat: 'meat',  price:   32, sell: true,  heal:  -15, desc: '물속에 그림자만 지나간다. 건지면 손에 무게만 남는다.' },
  { key: 'rumorConch',      name: '소문소라',                kind: '재료', cat: 'meat',  price:   12, sell: true,  heal:    0, desc: '귀에 대면 남이 한 말이 들린다. 누가 한 말인지는 안 알려 준다.' },
  { key: 'armorFish',       name: '철컹어',                  kind: '재료', cat: 'meat',  price:   28, sell: true,  heal:  -10, desc: '비늘이 사슬갑옷이다. 강에 빠진 기사가 하나 있었다고 한다.' },
  { key: 'clockCrab',       name: '클락크랩',                kind: '재료', cat: 'meat',  price:   30, sell: true,  heal:    0, desc: '등딱지 무늬가 시계다. 바늘이 느리게, 하지만 분명히 돈다.' },
  { key: 'coinCarp',        name: '코인카프',                kind: '재료', cat: 'meat',  price:   40, sell: true,  heal:    5, desc: '배 속에서 짤랑 소리가 난다. 갈라 볼 배짱이 있다면.' },
  { key: 'earAbalone',      name: '귀 전복',                 kind: '재료', cat: 'meat',  price:   30, sell: true,  heal:   20, desc: '사람 귀를 닮았다. 대고 말하면 가끔 대답이 온다.' },
  ...
  { key: 'raspberry',       name: '산딸기',                   kind: '재료', cat: 'fruit',  price:    2, sell: true,  heal: 5,                             desc: '베리 파수꾼단이 이름값 하는 순간.' },
  { key: 'blueberry',       name: '블루베리',                 kind: '재료', cat: 'fruit',  price:    3, sell: true,  heal: 5,                             desc: '한 움큼 먹으면 혀가 보라색이 된다.' },
  { key: 'grape',           name: '포도',                     kind: '재료', cat: 'fruit',  price:    4, sell: true,  heal: 5,   shop: true,  loot: false, desc: '먹어도 되고, 밟아도 된다. 밟으면 나중에 술이 된다.' },
  { key: 'lemon',           name: '레몬',                     kind: '재료', cat: 'fruit',  price:    4, sell: true,  heal: 2,   shop: true,  loot: false, desc: '시다. 너무 시다. 생선에 뿌리면 비린내가 달아난다.' },
  { key: 'keeperBerry',     name: '파수꾼 베리',              kind: '재료', cat: 'fruit',  price:   30, sell: true,  heal: 15,                            desc: '단원들만 아는 덤불에서 딴 베리. 알이 굵고, 단장이 제일 먼저 집어 간다.' },
  ...
  // ---- 장보기 둘째 배치. 시장에서 파는 흔한 것들 — 고기 다섯 · 채소 열둘 · 과일 셋 · 곡물 둘.
  //      고기는 날로 먹으면 조금 깎이고, 채소·과일은 조금 찬다. 흔한 채소는 던전에서도 난다.
  { key: 'pork',            name: '돼지고기',                kind: '재료', cat: 'meat',  price:    8, sell: true,  heal:  -3,  shop: true,  loot: false, desc: '기름이 적당히 붙었다. 날로 먹을 생각은 하지 말자.' },
  { key: 'beef',            name: '소고기',                  kind: '재료', cat: 'meat',  price:   14, sell: true,  heal:  -3,  shop: true,  loot: false, desc: '결이 곱다. 비싼 값을 하는 고기.' },
  { key: 'lamb',            name: '양고기',                  kind: '재료', cat: 'meat',  price:   12, sell: true,  heal:  -3,  shop: true,  loot: false, desc: '누린내가 난다고들 하지만, 그건 굽는 사람 탓이다.' },
  { key: 'duck',            name: '오리고기',                kind: '재료', cat: 'meat',  price:   11, sell: true,  heal:  -3,  shop: true,  loot: false, desc: '기름이 많다. 그 기름으로 감자를 볶으면 된다.' },
  { key: 'ham',             name: '햄',                      kind: '재료', cat: 'meat',  price:    9, sell: true,  heal:   6,  shop: true,  loot: false, desc: '이미 익혀 둔 고기. 썰기만 하면 된다.' },
  { key: 'greenOnion',      name: '대파',                    kind: '재료', cat: 'veg',   price:    3, sell: true,  heal:   2,  shop: true, desc: '길어서 장바구니 밖으로 삐져나온다.' },
  { key: 'tomato',          name: '토마토',                  kind: '재료', cat: 'veg',   price:    4, sell: true,  heal:   5,  shop: true, desc: '과일인지 채소인지로 한 번은 싸우게 된다.' },
  { key: 'eggplant',        name: '가지',                    kind: '재료', cat: 'veg',   price:    3, sell: true,  heal:   2,  shop: true,  loot: false, desc: '익히면 흐물흐물해진다. 그게 맛이다.' },
  { key: 'asparagus',       name: '아스파라거스',            kind: '재료', cat: 'veg',   price:    6, sell: true,  heal:   3,  shop: true,  loot: false, desc: '꼿꼿하다. 먹고 나면 다음 날 안다.' },
  { key: 'corn',            name: '옥수수',                  kind: '재료', cat: 'veg',   price:    3, sell: true,  heal:   4,  shop: true, desc: '한 줄씩 먹는 사람과 돌려 먹는 사람이 있다.' },
  { key: 'spinach',         name: '시금치',                  kind: '재료', cat: 'veg',   price:    3, sell: true,  heal:   4,  shop: true, desc: '먹으면 힘이 세진다는 말이 있다. 확인된 적은 없다.' },
  { key: 'cucumber',        name: '오이',                    kind: '재료', cat: 'veg',   price:    2, sell: true,  heal:   3,  shop: true, desc: '거의 물이다. 나머지는 향인데, 싫어하는 사람은 그 향을 싫어한다.' },
  { key: 'napaCabbage',     name: '배추',                    kind: '재료', cat: 'veg',   price:    3, sell: true,  heal:   2,  shop: true,  loot: false, desc: '속이 노랗다. 겉잎은 벗겨 낸다.' },
  { key: 'broccoli',        name: '브로콜리',                kind: '재료', cat: 'veg',   price:    4, sell: true,  heal:   3,  shop: true,  loot: false, desc: '작은 나무처럼 생겼다. 이걸 보고 숲을 떠올리는 사람이 있다.' },
  { key: 'sweetPotato',     name: '고구마',                  kind: '재료', cat: 'veg',   price:    3, sell: true,  heal:   4,  shop: true, desc: '구우면 달아진다. 물을 곁들이자.' },
  { key: 'paprika',         name: '파프리카',                kind: '재료', cat: 'veg',   price:    5, sell: true,  heal:   3,  shop: true,  loot: false, desc: '피망과 다르다는 사람과 같다는 사람이 있다.' },
  { key: 'beanSprout',      name: '콩나물',                  kind: '재료', cat: 'veg',   price:    2, sell: true,  heal:   1,  shop: true,  loot: false, desc: '머리부터 먹을지 꼬리부터 먹을지로 싸운 적은 없다. 아직은.' },
  { key: 'strawberry',      name: '딸기',                    kind: '재료', cat: 'fruit',  price:    5, sell: true,  heal:   6,  shop: true, desc: '씨가 겉에 있다. 과일 중에 드문 일이다.' },
  { key: 'peach',           name: '복숭아',                  kind: '재료', cat: 'fruit',  price:    6, sell: true,  heal:   7,  shop: true, desc: '껍질에 솜털이 있다. 만지면 간지럽다.' },
  ...
  { key: 'perilla',         name: '깻잎',                    kind: '재료', cat: 'veg',   price:    2, sell: true,  heal:   2,  loot: false, desc: '두 장씩 붙어서 떨어진다. 떼어 주는 사람을 두고 싸움이 난 적이 있다.' },
  { key: 'chili',           name: '고추',                    kind: '재료', cat: 'veg',   price:    4, sell: true,  heal:  -2,  loot: false, desc: '작은 게 맵다. 큰 것도 맵다.' },
  { key: 'turnip',          name: '순무',                    kind: '재료', cat: 'veg',   price:    3, sell: true,  heal:   2,  loot: false, desc: '무처럼 생겼는데 무보다 달다. 서리를 맞으면 더 달아진다.' },
  { key: 'beet',            name: '비트',                    kind: '재료', cat: 'veg',   price:    4, sell: true,  heal:   3,  loot: false, desc: '썰면 도마가 붉게 물든다. 손도 물든다. 한동안 간다.' },
  { key: 'taro',            name: '토란',                    kind: '재료', cat: 'veg',   price:    5, sell: true,  heal:  -1,  loot: false, desc: '맨손으로 껍질을 벗기면 가렵다. 장갑을 끼자.' },
  { key: 'pea',             name: '완두콩',                  kind: '재료', cat: 'grain', price:    3, sell: true,  heal:   2,  loot: false, desc: '꼬투리를 열면 한 줄로 앉아 있다. 하나씩 세고 싶어진다.' },
  ...
  { key: 'pear',            name: '배',                      kind: '재료', cat: 'fruit', price:    7, sell: true,  heal:   7,  loot: false, desc: '크고 둥글고 물이 많다. 깎다 보면 손목까지 단물이 흐른다.' },
  ...
  { key: 'belladonna',      name: '까망 반들열매',            kind: '재료', cat: 'fruit',  price:    6, sell: true,  heal: -25, poison: 2,              desc: '까맣고 반들반들한 열매. 달큰한 향이 코끝을 간지럽힌다. 이상하게 새들은 이걸 안 쪼아 먹는다.' },
