 ...
 *
 * 판 하나가 정상적으로 만들 수 있는 최대 손실이 한 스택(1000)이므로, 그보다 더
 * 내려간다는 것은 **같은 칩을 두 판에서 겹쳐 걸었다**는 뜻이다(봇이 막지만 여기서
 * 한 번 더 본다). **0 으로 깎지는 않는다** — 깎으면 없던 칩이 생기고, 총합 보존으로
 * 버그를 잡는 길이 막힌다. 거절하고 로그에 남기는 편이 낫다.
 */
