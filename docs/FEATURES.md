...
둘 다 **스레드에서 돈다.** 주사위·카드·대사가 오가는 통에 원래 채널이 묻히지 않게.
...
- **블랙잭** — 카지노 "bard" 이고 **미겔이 딜러**다. 미겔을 손님 자리에 앉히면 `npc` 가
  ...
  칩은 지금 **판마다 초기화**된다(영구 저장 계획은 아래 "아직 안 만든 것").
...
**대사**는 두 게임의 정책이 다르다. 요트는 Gemini 로만 짓고 실패하면 그냥 비운다.
블랙잭은 한 핸드에 대사 자리가 스무 번 넘게 와서 전부 API 로 갈 수 없으므로,
**Gemini 로 먼저 지어 보고 안 되면 손으로 쓴 `bot/data/casino-lines.json`(600여 줄) 로
물러선다.** 한도가 찼든 API 가 죽었든 판은 늘 말이 있는 채로 돈다.
...
- **칩 영구 저장.** 블랙잭 칩은 지금 판마다 초기화된다. 서버에 저장해 `/출첵`·`/칩 양도` 까지
  붙이는 계획은 `docs/CASINO_CHIPS.md` 에 적어 뒀다. 봇 쪽 이음매
  (`casino/wallet.js` 의 `load`/`commit`)는 이미 만들어져 있다.
