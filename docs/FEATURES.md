...
| `/블랙잭 시작 · 판 · 그만` | 누구나 | 없음 (Gemini) |
| `/홀덤 시작 · 판 · 그만` | 누구나 | 없음 (Gemini) |
| `/주사위` `/뽑기` | 누구나 | 없음 |
...
- **텍사스 홀덤** — 노리밋. 최대 4자리, 두 자리부터. 블라인드 10/20 에 앉으면 1000칩(50BB).
  홀덤에는 딜러석이 없어서(카드는 봇이 돌린다) **미겔도 손님으로 앉고**, 판을 여는 인사만
  bard 주인으로서 겸한다. 레이즈 금액은 최소·½팟·팟·올인 버튼으로 고른다.

블랙잭·홀덤의 칩은 지금 **판마다 초기화**된다(영구 저장 계획은 아래 "아직 안 만든 것").
...
규칙을 본다(**칩 총합 보존**과 베팅 라운드가 닫히는지). 사이드팟과 찹이 이 게임에서 제일
...
- **칩 영구 저장.** 블랙잭·홀덤 칩은 지금 판마다 초기화된다. 서버에 저장해
  `/출첵`·`/칩 양도` 까지 붙이는 계획은 `docs/CASINO_CHIPS.md` 에 적어 뒀다.
  봇 쪽 이음매(`casino/wallet.js` 의 `load`/`commit`)는 이미 만들어져 있고, 카지노
  게임 셋이 그걸 공유하므로 한 번 붙이면 셋 다 저장된다.
