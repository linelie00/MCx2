...
import { FARM_CROPS } from './items.js';
...
/** 농장 작물 전종(희귀 · 과수 포함). 「살아 있는 식물도감」의 분모 — 서버 작물표와 같다(check-farm). */
export const FARM_TOTAL = FARM_CROPS.length;
...
export const TITLES = [
  ...
  // ---------------------------------------------------------------- 농장 — 서버가 적는 `farm…` 전적(controllers/farmController.js)
  { key: 'sprout', name: '새싹 농부', tier: 1, cond: '농장에서 처음 거두면', group: '농장', desc: '제 손으로 키운 것을 처음 거뒀다.', when: (s) => n(s, 'farmHarvest') >= 1 },
  { key: 'wateringCan', name: '물뿌리개', tier: 1, cond: '물주기 30번', group: '농장', desc: '서른 번째쯤부터 손목이 먼저 기울어진다.', when: (s) => n(s, 'farmWater') >= 30 },
  { key: 'goodNeighbor', name: '이웃사촌', tier: 2, cond: '남의 농장에 물주기 20번', group: '농장', desc: '먼 친척보다 가까운 이웃.', when: (s) => n(s, 'farmHelp') >= 20 },
  { key: 'stoneBreaker', name: '돌 깨는 사람', tier: 1, cond: '돌·바위 50개 치우기', group: '농장', desc: '밭은 돌을 치운 만큼 넓어진다.', when: (s) => n(s, 'farmClear') >= 50 },
  { key: 'grainReader', name: '결을 읽는 자', tier: 2, cond: '바위를 첫 방에 10번 깨기', group: '농장', desc: '한 번 보고, 한 번에 쪼갠다.', when: (s) => n(s, 'farmPerfect') >= 10 },
  { key: 'villageFarmer', name: '마을 농부', tier: 1, cond: '농장 Lv.5', group: '농장', desc: '마을에서 이름을 대면 밭 자리를 안다.', when: (s) => n(s, 'farmLevel') >= 5 },
  { key: 'greatFarmer', name: '대농', tier: 3, cond: '농장 Lv.10', group: '농장', desc: '아홉 밭이 전부 이 사람 것이다.', when: (s) => n(s, 'farmLevel') >= 10 },
  { key: 'goodHarvest', name: '풍년', tier: 2, cond: '1,000칸 거두기', group: '농장', desc: '광이 모자라 이웃에 나눠 줬다.', when: (s) => n(s, 'farmHarvest') >= 1000 },
  { key: 'goldenHand', name: '황금 손', tier: 2, cond: '★★★ 작물 5종 거두기', group: '농장', desc: '심기만 하면 윤이 난다.', when: (s) => n(s, 'farmStar3Kinds') >= 5 },
  { key: 'giantGrower', name: '대왕 작물', tier: 3, cond: '대왕 작물 1번', group: '농장', desc: '아홉 칸이 하나로 합쳐졌다. 수레가 부서졌다.', when: (s) => n(s, 'farmGiant') >= 1 },
  { key: 'botanist', name: '식물학자', tier: 2, cond: '농장 도감 30종', group: '농장', desc: '잎만 보고 이름을 댄다.', when: (s) => n(s, 'farmBookKinds') >= 30 },
  { key: 'herbarium', name: '살아 있는 식물도감', tier: 3, cond: `농장 도감 ${FARM_TOTAL}종 모두`, group: '농장', desc: '이 땅에서 자라는 것은 다 길러 봤다.', when: (s) => n(s, 'farmBookKinds') >= FARM_TOTAL },
  { key: 'wellEquipped', name: '장비빨', tier: 2, cond: '설비 10개 놓기', group: '농장', desc: '농사는 장비가 반이다.', when: (s) => n(s, 'farmEquip') >= 10 },
  { key: 'earRinging', name: '귀가 멍멍', tier: 1, cond: '귀마개 없이 비명 뿌리를 거두면', group: '농장', desc: '한동안 아무 소리도 안 들렸다.', when: (s) => n(s, 'farmScream') >= 1 },
